...
int main(void){
  //setup
  BIT_CLEAR(DDRB, BUTTON_PIN); //clear pin on DDRB (set as 0) to configure as input
  BIT_SET(PORTB, BUTTON_PIN); //set pin on PORTB as 1 to activate pull-up resistor
  //main loop
  while(1){
    _delay_ms(100);
    if(BIT_CHECK(PINB, BUTTON_PIN) == 0){
      LED_ON = !LED_ON;
      if(LED_ON == true){
        BIT_SET(PORTB, LED_PIN); //set pin 0 on port B (aka freenove pin 8) as 1 and turn led on
